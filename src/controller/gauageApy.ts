...
const rewardRemaining = async (guage) => {
  let rewardsLeftObj = {};
  ...
  for (var i = 0; i < Number(guageRewardslength); i++) {
    ...
    const rewardsLeft = (await guage.left(rewardsToken)).div(
      rewardtokenDecimal
    );
...
const getAPR = async (
  ...
  const solidRewardinUSD =
    rewardsleftResponse["SOLID"] * collateralPrices.SOLID;
...
const fetchAPRs = async () => {
  ...
  return {
    "0x0AEfb132a53feD4e555F01e52db40C8490c0da1d": {
      min: String((daiMahaApr * 1) / 5),
      max: String(daiMahaApr),
    },
    "0x7D4f9CAA114d55Bc2e77ba7B940d60D3e1e64fa2": {
      min: String((daiSolidApr * 1) / 5),
      max: String(daiSolidApr),
    },
    "0x281eCb4D21508e1B029d0FC005E7fd9eED2Af86a": {
      min: String((mahaSolidApr * 1) / 5),
      max: String(mahaSolidApr),
    },
  };
};
