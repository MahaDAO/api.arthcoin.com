...
const arthxarth = async () => {
    ...
    console.log('quaterlyRewards', quaterlyRewards);
    let oneDayRewardAmount = (quaterlyRewards / 90) * LPUSD //* RewardToUSD
    let halfYearlyRewardAmount = (oneDayRewardAmount * 180)

    // let APY_inPOOLToken = ((halfYearlyRewardAmount * 2) / 360) * 100
    // console.log(APY_inPOOLToken);

    const poolTokenAddress = await stakeArthxArth.methods.rewardsToken().call()
    console.log(poolTokenAddress);
    let poolTokenArthxBalance = await arthx.methods.balanceOf(poolTokenAddress).call()
    console.log('poolTokenArthxBalance', poolTokenArthxBalance);
...
//StakeARTHMAHA staking contract
